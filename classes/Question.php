	  	...
	  	public function addtruefalsequestion($post,$file,$gid,$gtoken){
	        ...
	        if(empty($tfqtitle) || empty($tffiop) || empty($tfsiop)|| empty($tfcs)){
	  		...
	  		if(!empty($file_name)) {
      	  	...
      	  	public function addtheoryquestion($post,$file,$gid,$gtoken){

	  		$thqtitle   = $this->fm->validation($post['thqtitle']);
	  		$thcs     = $this->fm->validation($post['thcs']);
	  		
	  		$gid         = $this->fm->validation($gid);

	  		$thqtitle  = mysqli_real_escape_string($this->db->link, $thqtitle);
	  	
	  		$thcs    = mysqli_real_escape_string($this->db->link, $thcs);
	  		$gid    = mysqli_real_escape_string($this->db->link, $gid);
	  		$gtoken     = mysqli_real_escape_string($this->db->link, $gtoken);
	        ...
	        if(empty($thqtitle) ||  empty($thcs)){
	  		...
	  		if(!empty($file_name)) {
		        ...
		        move_uploaded_file($file_temp, $uploaded_directory_Imagename);
		    	$query = "INSERT into groupquestions(questionTitle,thcs,image,group_id,group_token) VALUES (?,?,?,?,?)";
					 $type = array('sssssis');
		             $result = $this->db->insert($query,$type,array(&$thqtitle,&$thcs,&$uploaded_directory_Imagename,&$gid,&$gtoken));
	                   if($result){
	             		  $msg = "Question Added Successfully...";
			            return $msg;
	             	}else
	        	...
	        	$query = "INSERT into groupquestions(questionTitle,thcs,group_id,group_token) VALUES (?,?,?,?)";
					 $type = array('ssssis');
		             $result = $this->db->insert($query,$type,array(&$thqtitle,&$thcs,&$gid,&$gtoken));
	                   if($result){
	             		  $msg = "Question Added Successfully...";
			            return $msg;
	             	}else
      	  	...
      	  	public function showtheoryquestion($gid){
	  		$tff="";
